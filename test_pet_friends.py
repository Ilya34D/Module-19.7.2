...
def test_get_api_key_for_invalid_user(email=invalid_email, password=valid_password):
    """" Проверяем, что запрос ключа от несуществующего пользователя возвращает
     статус 403"""
...
def test_add_new_pet_with_valid_data(name='Матильда', animal_type='кошка',
                                     age='3', pet_photo='images/kohka.jpg'):
...
def test_add_new_pet_with_invalid_age(name='Ириска', animal_type='терьер',
                                      age="invalid_age", pet_photo='images/P1040103.jpg'):
...
def test_successful_delete_self_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "бурундук", "бурук", '3', "images/chipmunk.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
def test_successful_update_self_pet_info(name='Плюшка', animal_type='дворняга',
                                         age='99'):
    ...
    if len(my_pets['pets']) > 0:
        ...
        raise Exception("Pets are missing, need to add them")


def test_add_new_pet_no_photo_with_valid_data(name='cat_no_photo2', animal_type='kotik2',
                                              age='5'):
